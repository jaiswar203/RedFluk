...
import { getTvShowById } from "../../../action/movies/TvShow";

const TvDetail = () => {
  const { detail } = useSelector((state) => state.tvshow);
  const { id } = useParams();
      ...
      <div className="movies-links">
        <div className="movies-links-google">
          ...
          <div className="movies-links-link">
            <Link to={`/secondary/download/tvshow/${id}?quality=480p`}>
              <button>
                {" "}
                <i className="fas fa-download"></i> 480p
              </button>
            </Link>
            <Link to={`/secondary/download/tvshow/${id}?quality=720p`}>
              <button>
                {" "}
                <i className="fas fa-download"></i> 720p
              </button>
            </Link>
            <Link to={`/secondary/download/tvshow/${id}?quality=1080p`}>
              <button>
                {" "}
