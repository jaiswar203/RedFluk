...
import { createWebShow, updateWebShow } from "../../../../action/movies/WebShow";

const Cards = ({ data, setData, currentId, setCurrentId }) => {
  const dispatch = useDispatch();
  const movies = useSelector((state) =>
    currentId ? state.webshow.movies.find((p) => p._id === currentId) : null
  );
  ...
  const handleSubmit = (e) => {
    ...
    if (currentId) {
      dispatch(updateWebShow(currentId, {...movieData,name:user?.result?.name}));
    } else {
      dispatch(createWebShow({...movieData,name:user?.result?.name,datatype:'webseries'}));
    }
    clear();
  };
