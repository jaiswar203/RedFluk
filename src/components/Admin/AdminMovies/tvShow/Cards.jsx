...
import { createTvShow, updateTvShow } from "../../../../action/movies/TvShow";

const Cards = ({ data, setData, currentId, setCurrentId }) => {
  const dispatch = useDispatch();
  const movies = useSelector((state) =>
    currentId ? state.tvshow.movies.find((p) => p._id === currentId) : null
  );
  ...
  const handleSubmit = (e) => {
    ...
    if (currentId) {
      dispatch(updateTvShow(currentId, {...movieData,name:user?.result?.name}));
    } else {
      dispatch(createTvShow({...movieData,name:user?.result?.name,datatype:'tvshow'}));
    }
    clear();
  };
