...
import { createAnimeSeries, updateAnimeSeries } from "../../../../../action/movies/animated/WebShow";

const Cards = ({ data, setData, currentId, setCurrentId }) => {
  const dispatch = useDispatch();
  const movies = useSelector((state) =>
    currentId ? state.animatedWebshow.movies.find((p) => p._id === currentId) : null
  );
  ...
  const handleChange = (e) => {
    if (data !== undefined) {
      setMovieData({
        ...
        title: data.Title,
        genre: data.Genre ? data?.Genre.split(",").map((data)=>data.trim()) : null ,
  ...
  const handleSubmit = (e) => {
    ...
    if (currentId) {
      dispatch(updateAnimeSeries(currentId, {...movieData,name:user?.result?.name}));
    } else {
      dispatch(createAnimeSeries({...movieData,name:user?.result?.name,datatype:'animeseries'}));
    }
    clear();
  };
            ...
            <div className="trailer-detail">
              ...
              <div className="trailer-detail-input">
                ...
                <input
                  ...
                  onChange={(e)=>setMovieData({...movieData,gdrive1:e.target.value.split(',')})}
                  required
                />
              </div>
              <div className="trailer-detail-input">
                ...
                <input
                  ...
                  placeholder="gdrive Link"
                  name="gdrive2"
                  onChange={(e)=>setMovieData({...movieData,gdrive2:e.target.value.split(',')})}
                  required
                />
              </div>
              <div className="trailer-detail-input">
                ...
                <input
                  ...
                  onChange={(e)=>setMovieData({...movieData,gdrive3:e.target.value.split(',')})}
                  required
                />
              </div>
              {/* <div className="trailer-detail-input">
